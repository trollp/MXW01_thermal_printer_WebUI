...
function renderForecast(job) {
  ...
  let days = (job.days || []).slice(0, Math.max(1, Number(job.maxDays) || 7)).map((d) => {
    let label = d.label;
    if (!label && d.date) { const dt = new Date(d.date); if (!Number.isNaN(dt.getTime())) label = `${DAYS[dt.getDay()]} ${dt.getDate()}`; }
    const high = Number(d.high ?? d.temperature), low = Number(d.low ?? d.templow);
    return { label: label || "", condition: d.condition, high, low: Number.isFinite(low) ? low : high, precip: Number(d.precipitation) || 0, prob: d.precipitationProbability, wind: d.wind ?? d.wind_speed, windUnit: d.windUnit || "m/s" };
  ...
  if (job.now) {
    ...
    ctx.font = f(13);
    const text = job.now.text || String(job.now.condition || "").replace(/-/g, " ");
