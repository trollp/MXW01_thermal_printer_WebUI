...
class PrinterManager extends EventEmitter {
  constructor({ idleDisconnectSec = 120, scanTimeoutSec = 30, address = "", adapter = "hci0", keepAlive = false, keepAliveIntervalSec = 60, deferDir = null, log = () => {} } = {}) {
    super();
    this.deferDir = deferDir; // parked jobs are also written here so they survive restarts
  ...
  async printOrDefer(imageData, popts, label = "print") {
    try {
      ...
      const job = { imageData, popts, label, at: Date.now(), id: `${Date.now()}-${Math.random().toString(36).slice(2, 7)}` };
      this.deferred.push(job);
      while (this.deferred.length > this.deferMax) this._forget(this.deferred.shift());
      this._persist(job);
  ...
  async _flushDeferred() {
    ...
    for (const j of jobs) if (j.at < cutoff) this._forget(j);
    for (const j of fresh) {
      try { await this.print(j.imageData, j.popts, j.label); this._forget(j); }
      catch (err) { this.log(`parked job "${j.label}" failed: ${err.message}`); this.deferred.push(j); }
    }
    this.emit("change");
  }

  /** Drop all parked jobs (memory + disk). */
  clearDeferred() {
    const n = this.deferred.length;
    for (const j of this.deferred.splice(0)) this._forget(j);
    this.emit("change");
    return n;
  }

  // --- parked-job persistence: <deferDir>/<id>.png (the bitmap) + <id>.json (options) ---
  _persist(job) {
    if (!this.deferDir || !job.id) return;
    try {
      fs.mkdirSync(this.deferDir, { recursive: true });
      const c = createCanvas(job.imageData.width, job.imageData.height);
      c.getContext("2d").putImageData(job.imageData, 0, 0);
      fs.writeFileSync(path.join(this.deferDir, job.id + ".png"), c.toBuffer("image/png"));
      fs.writeFileSync(path.join(this.deferDir, job.id + ".json"), JSON.stringify({ popts: job.popts, label: job.label, at: job.at }));
    } catch (err) { this.log("could not persist parked job: " + err.message); }
  }
  _forget(job) {
    if (!this.deferDir || !job.id) return;
    for (const ext of [".png", ".json"]) { try { fs.unlinkSync(path.join(this.deferDir, job.id + ext)); } catch {} }
  }
  async loadDeferred() {
    if (!this.deferDir || !fs.existsSync(this.deferDir)) return 0;
    let n = 0;
    for (const f of fs.readdirSync(this.deferDir).filter((f) => f.endsWith(".json")).sort()) {
      const id = f.slice(0, -5);
      try {
        const meta = JSON.parse(fs.readFileSync(path.join(this.deferDir, f), "utf8"));
        const img = await loadImage(path.join(this.deferDir, id + ".png"));
        const c = createCanvas(img.width, img.height);
        const ctx = c.getContext("2d"); ctx.drawImage(img, 0, 0);
        this.deferred.push({ imageData: ctx.getImageData(0, 0, img.width, img.height), popts: meta.popts, label: meta.label, at: meta.at, id });
        n++;
      } catch (err) { this.log(`dropping unreadable parked job ${id}: ${err.message}`); this._forget({ id }); }
    }
    if (n) this.log(`${n} parked job(s) restored from disk`);
    this.emit("change");
    return n;
  }
...
function isLinkError(err) {
  return /characteristic not found|not connected|disconnected|No peripheral|Failed to connect|Device scan timeout|printer not (found|reachable)|ConnectDevice timed out/i.test(err && err.message || "");
}
