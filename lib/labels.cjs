...
const ICONS = {
  ...
  cheese(ctx, s) {
    ctx.lineWidth = 3;
    ctx.beginPath(); ctx.moveTo(s * 0.1, s * 0.8); ctx.lineTo(s * 0.9, s * 0.8); ctx.lineTo(s * 0.9, s * 0.45); ctx.lineTo(s * 0.1, s * 0.3); ctx.closePath(); ctx.stroke();
    for (const [x, y, r] of [[0.35, 0.55, 0.06], [0.62, 0.65, 0.05], [0.5, 0.42, 0.04]]) { ctx.beginPath(); ctx.arc(s * x, s * y, s * r, 0, Math.PI * 2); ctx.stroke(); }
  },
  heart(ctx, s) {
...
async function renderLabel(job) {
  ...
  const textBlockH = titleLines.length * titleLH + (subLines.length ? subLines.length * subLH + 4 : 0) + smallH;
