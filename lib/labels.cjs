...
const ICONS = {
  ...
  cheese(ctx, s) {
    // block wedge: front face with bites on the left and bottom edges, sloped top
    // face with a rounded back corner and a bite, holes on both faces
    const P = (x, y) => [s * x, s * y];
    const L = 0.08, R = 0.92, T = 0.44, B = 0.9;          // front face box
    const slopeA = [R, 0.44], slopeB = [0.62, 0.12];        // top face: from ridge-right corner up to the peak
    ctx.lineWidth = 3.2; ctx.lineJoin = "round";
    ctx.beginPath();
    ctx.moveTo(...P(L, T));
    // left edge with a bite
    ctx.lineTo(...P(L, 0.56)); ctx.arc(s * L, s * 0.65, s * 0.09, -Math.PI / 2, Math.PI / 2, false); ctx.lineTo(...P(L, B));
    // bottom edge with a bite
    ctx.lineTo(...P(0.3, B)); ctx.arc(s * 0.41, s * B, s * 0.11, Math.PI, 2 * Math.PI, false); ctx.lineTo(...P(R, B));
    // right edge up, rounded back corner into the peak
    ctx.lineTo(...P(R, 0.34)); ctx.quadraticCurveTo(s * R, s * 0.1, s * slopeB[0], s * slopeB[1]);
    // sloped edge down to the front-left corner, with a bite in it
    {
      const dx = L - slopeB[0], dy = T - slopeB[1], len = Math.hypot(dx, dy), ux = dx / len, uy = dy / len;
      const a = Math.atan2(uy, ux), t = 0.42, r = 0.065;
      const cx = slopeB[0] + dx * t, cy = slopeB[1] + dy * t;
      ctx.lineTo(...P(cx - ux * r, cy - uy * r));
      ctx.arc(s * cx, s * cy, s * r, a + Math.PI, a, false);
      ctx.lineTo(...P(L, T));
    }
    ctx.closePath(); ctx.stroke();
    // ridge between the faces
    ctx.beginPath(); ctx.moveTo(...P(L, T)); ctx.lineTo(...P(R, T)); ctx.stroke();
    // holes
    ctx.lineWidth = 2.8;
    const hole = (x, y, rx, ry = rx) => { ctx.beginPath(); ctx.ellipse(s * x, s * y, s * rx, s * ry, 0, 0, Math.PI * 2); ctx.stroke(); };
    ctx.fillStyle = "white";
    ctx.beginPath(); ctx.ellipse(s * 0.5, s * T, s * 0.065, s * 0.045, 0, 0, Math.PI * 2); ctx.fill(); ctx.stroke(); // straddles the ridge
    ctx.fillStyle = "black";
    hole(0.3, 0.62, 0.05); hole(0.52, 0.73, 0.03); hole(0.72, 0.7, 0.05, 0.035); hole(0.68, 0.24, 0.03);
  },
  heart(ctx, s) {
